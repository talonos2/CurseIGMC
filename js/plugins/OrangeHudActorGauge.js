...
if (Imported.OrangeHudActorGauge === undefined) {
  ...
  OrangeHudActorGauge.getValue = function(variableData) {
    ...
    {
      var actorData = $gameParty.members()[variableData.ActorIndex];
      if (!actorData)
      {
        return 0;
      }
      return actorData.hp+'/'+actorData.mhp+'/'+actorData.mp+'/'+actorData.mmp+'/'+actorData.currentExp();
    } else {
      return 0;
    }
  };
  ...
  OrangeHudActorGauge.getActorValue = function(variableData, expression) {
    ...
    if (members.length > variableData.ActorIndex) {
      ...
      expression = expression.replace(/<exp>/gi, actorData.currentExp()-actorData.expForLevel(actorData.level));
      expression = expression.replace(/<mexp>/gi, actorData.expForLevel(actorData.level+1)-actorData.expForLevel(actorData.level));
